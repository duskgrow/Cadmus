...
impl AgentLoop {
    ...
    /// pairs, and pushes the tool messages onto the history. A batch runs
    ...
    async fn dispatch_tools(
        ...
        if all_safe {
            self.dispatch_parallel(&calls, messages, root_span, turn)
                .await?;
        } else {
            for call in &calls {
                self.dispatch_one(call, messages, root_span, turn).await?;
            }
        }
        Ok(())
    }

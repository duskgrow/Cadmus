...
编码工具（`read_file`、`grep`、`list_dir`、`write_file`、`edit_file`、`todo_write`、`skill`）限制在当前目录内；写操作默认拒绝，无人值守使用时传 `-y` 批准。工作区指令文件（`AGENTS.md`——用户全局加上从根到当前目录的祖先链）会加载进系统提示，技能（`.agents/skills/*/SKILL.md`——工作区加上用户全局 `~/.agents/skills`）以按需激活的目录形式进入提示。完整选项见 `cadmus --help`。

...
- Time zones: times describing the user's world (e.g. the trailer clock) render in the machine's local timezone; machine-facing records (event timestamps, trace ids) stay epoch/UTC; calendar math is never hand-rolled — the `time` crate owns it (maintainer, 2026-09-10).
- When the maintainer asks what a piece of code means or why it exists, the explanation lands as a brief comment at that spot in the same change — chat answers rot. The same write-back covers decisions and supplementary information from any discussion: a comment at the spot, or the one owning doc (ADRs own decisions, tools.md the tool catalog, open-items.md findings awaiting their consumer) — one home per fact, never duplicated (SSOT). Comments still pass the SSOT bar: intent, constraints and tradeoffs, never a restatement of mechanism.
...
- Universal standards first: where a cross-vendor spec or convention exists (agents.md, agentskills.io, `.agents/skills`, MCP), adopt it as-is; Cadmus extensions live only in the spec's own extension points (e.g. SKILL.md `metadata` with `cadmus.*` keys, ADR-0006 item 3), never in a parallel private format. No mechanical gate — PR review enforces.
